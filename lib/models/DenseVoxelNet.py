...
def init_weights(m):
    """
    The weights were randomly initialized with a Gaussian distribution ($\mu=0, \sigma=0.01$)
    """
...
class DenseVoxelNet(BaseModel):
    ...
    def test(self, device='cpu'):
        ...
        summary(self.to(torch.device(device)), (self.in_channels, 8, 8, 8), device='cpu')
        b, c = self.forward(a)
...
#     densevoxelnet = DenseVoxelNet(in_channels=1, classes=3)
#     densevoxelnet.test()