# /*
#  * @Author: anonymous
#  * @Date: 2021-01-13 16:12:19
#  * @Last Modified by:   anonymous
#  * @Last Modified time: 2021-01-13 16:12:19
#  */
import torch
...
class Upsampling(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization='none'):
        ...
        ops = []
        ops.append(nn.Upsample(scale_factor=stride,
...
class MultiOrganNet(BaseModel):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        super(MultiOrganNet, self).__init__()
    ...
    @classmethod
    def test(cls, device='cpu'):
        multiorgan = cls(n_channels=3, n_classes=2)
        input_tensor = torch.rand(1, 3, 32, 32, 32)
        output = multiorgan(input_tensor)
        print("Output shape is {output.shape}")
        device = torch.device(device)
        summary(multiorgan.to(device), (3, 32, 32, 32), device='cpu')
        print("MultiOragnNet test is complete~")
